...
from ..path_services.pathfinder import PathFinderV2 as PathFinder
from ..path_services.pathfinder import SCIONPath
from ..path_services.pathprobe import PathMetrics, PathProbe
...
class AlgorithmHarness:
    ...
    def load_algorithm_plugin(
        self, module_path: str, class_name: str, config: Dict = None
    ):
    ...
    def run_experiment(
        ...
        with ProcessPoolExecutor(max_workers=n_jobs) as executor:
            results = list(executor.map(self._run_single_experiment, work_items))
    ...
    def _run_single_experiment(self, work_item: tuple) -> List[FlowResult]:
        ...
        for i, flow in enumerate(flow_trace):
            ...
            # Get current metrics
            metrics = self.path_probe.probe_batch(paths, flow.start_time, noisy=True)
            ...
            selected_path = algorithm.select_path(
                flow.src, flow.dst, feasible_paths, feasible_metrics, flow
            )
    ...
    def generate_flow_trace(
        self, topology_path: Path, num_flows: int, time_slots: int
    ) -> List[FlowRequest]:
    ...
    def compute_metrics(self, results_df: pd.DataFrame) -> Dict:
        ...
        for algo in results_df["algorithm"].unique():
            ...
            if len(successful) > 0:
                latencies = [
                    m.latency_ms for m in successful["metrics"] if m is not None
                ]
                if latencies:
