...
# The intents used. These MUST match the profiles the conditional
...
#                                 is unit-tested in tests/test_intent_cond_eval.py.
...
def _write_ablation_table(
    ...
    lines = [
        "% Auto-generated by src/pipeline/intent_cond_eval.py -- do not edit by hand.",
...
def run_intent_alignment(
    ...
    *,
    ...
    ``CONDITIONAL_CHECKPOINTS``. Reports whichever variant it ships.
    """
    log = progress or (lambda _m: None)
    ctx = run_context or load_run_context(run_path)
    topology_data, path_store, link_states, pair_pool, _goodput_cap = ctx
...
def run_probing_ceiling(
    ...
    *,
    ...
    drives ``probing_quality.csv`` / ``ceiling_by_congestion.csv`` (the schema the
     figures read); with more than one, an additional
