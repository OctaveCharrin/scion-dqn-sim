...
class PathFinderV2:
    ...
    def __init__(
        self, topology_path: Path, segment_store_path: Path, link_table_path: Path
    ):
        """
        Args:
        ...
        self.as_to_isd = {
            row["as_id"]: row["isd"] for _, row in self.node_df.iterrows()
        }
    ...
    def get_paths(
        self, src: int, dst: int, k: int = 16, policy: str = "min-lat"
    ) -> List[SCIONPath]:
        """
    ...
    def _find_direct_path(self, src: int, dst: int) -> Optional[SCIONPath]:
        ...
        return SCIONPath(
            src=src, dst=dst, hops=tuple(hops), path_type="direct", segment_ids=()
        )
    ...
    def _find_inter_isd_paths(
        self, src: int, dst: int, src_isd: int, dst_isd: int
    ) -> List[SCIONPath]:
            ...
            if nx.has_path(G, src, dst):
                ...
                for path in paths:
                    ...
                    segment_store["core"].append(
                        {"path": segment, "type": "core", "src": src, "dst": dst}
                    )
...
def create_segment_from_path(G, path):
    ...
    for i in range(len(path)):
        ...
        segment.append({"as": as_id, "ingress": ingress_if, "egress": egress_if})
...
def reverse_segment(segment):
    ...
    for hop in reversed(segment):
        reversed_seg.append(
            {"as": hop["as"], "ingress": hop["egress"], "egress": hop["ingress"]}
        )
    return reversed_seg
