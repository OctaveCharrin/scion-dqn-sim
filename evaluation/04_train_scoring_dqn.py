#!/usr/bin/env python3
"""Train the **simple** path-scoring DQN (flat MLP per path, uniform replay).

Uses dict observations ``{"global": (G,), "paths": (N, P)}`` so Q-values are
per-path from features, not from a fixed index. For Dueling + PER + Double DQN,
see ``04_train_scoring_enhanced_dqn.py``.
"""
...
# Sibling helper (``evaluation/`` is not always an importable package name).
...
max_paths_pool = int(
    max(
        max(pair_paths_count.values(), default=1),
        int(selected_pair.get("max_num_paths", 1) or 1),
        1,
    )
)
print(f"\nTraining Simple Path-Scoring DQN across {len(pair_pool)} AS pair(s)")
print(f"  Max paths in pool (info only): {max_paths_pool}")
...
print("\nSimple Path-Scoring DQN configuration:")
...
model_file = run_path / "dqn_scoring_simple_model.pth"
torch.save(
    {
        ...
        "max_num_paths_pool": max_paths_pool,
    },
    model_file,
)
print(f"\nSimple path-scoring model saved to: {model_file}")
...
with open(run_path / "dqn_scoring_simple_training_stats.json", "w") as f:
    json.dump(training_stats, f, indent=4)
print(
    f"Simple path-scoring training stats saved to: "
    f"{run_path / 'dqn_scoring_simple_training_stats.json'}"
)
