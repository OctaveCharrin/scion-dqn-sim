...
- **Visualize Topology**: `uv run python evaluation/visualize_topology.py <run_dir> --mode full`
- **Run individual step**: `uv run python 04_train_dqn.py <run_dir>` (re-uses existing run artifacts)

## Development Conventions

- **Evaluation Pipeline**: Experiments must follow the numbered scripts in `evaluation/`. Each script accepts a run directory as its first argument and uses `_common.resolve_run_dir()` for consistency.
...
- **Imports**: Use relative imports within `src` or absolute imports starting with `src.`. Scripts in `evaluation/` add the repo root to `sys.path` via `from _common import ...`.

## Key Files & Directories

- `evaluation/run_full_evaluation.py`: Main entry point for the experiment pipeline.
- `evaluation/_common.py`: Shared utilities for run directory resolution and subprocess execution.
